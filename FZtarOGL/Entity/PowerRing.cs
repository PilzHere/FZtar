...
namespace FZtarOGL.Entity
{
    public class PowerRing : Entity
    {
        ...
        public PowerRing(Screen.Screen screen, AssetManager assMan, Vector3 position)
        {
            _screen = screen;
            _assMan = assMan;

            _modelRing = _assMan.Ring02Model;
            _modelBillboard = _assMan.BillboardFZ16Model;
            ...
            boxf = new BoundingBoxFiltered(this, min, max, BoxFilters.FilterRingPower, BoxFilters.MaskRingPower);
        ...
        public override void Tick(float dt)
        {
            ...
            if (!powerGiven) _screen.TransparentModels.Add(new TransparentModel(_modelBillboard, ModelTransBillboard));
        }
        ...
        public override void OnCollision(int filter, float dt)
        {
            if (filter == BoxFilters.FilterPlayerShip)
            {
                if (!powerGiven)
                {
                    SoundEffectInstance sfx = _assMan.SfxPowerUp.CreateInstance();
